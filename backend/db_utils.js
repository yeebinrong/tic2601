...
const getHomePagePosts = (currentUser, sortBy) => {
    return POOL.query(
        `WITH following_communities AS
            ...
            COALESCE(SUM(f.favour_point), 0) AS fav_point, fp.favour_point AS is_favour, COUNT(c.comment_id) AS comment_count, u.profile_picture
            ...
            GROUP BY fc.community_name, fc.user_name, p.user_name, p.post_id, p.date_created, p.date_deleted, p.title, p.flair, p.url, p.view_count, fp.favour_point, c.comment_id, u.profile_picture
...
const deletePost = (community,username,title,currentUser) => {
    return POOL.query(`UPDATE posts SET date_deleted = CURRENT_TIMESTAMP WHERE community_name = $1 AND user_name = $2 AND title = $3 AND user_name = $4;`,
        [
...
const retrieveCommunityPostsDB = (community, currentUser) => {
    return POOL.query(
           ...
           COALESCE(SUM(f.favour_point), 0) AS fav_point, fp.favour_point AS is_favour, COUNT(c.comment_id) AS comment_count, p.url, u.profile_picture
               ...
               LEFT JOIN comments c ON c.post_id = f.post_id AND c.community_name = p.community_name
               LEFT JOIN users u ON u.user_name = p.user_name
               GROUP BY oc.community_name, p.user_name, p.post_id, p.date_created, p.date_deleted, p.title, p.flair, p.url, p.view_count, fp.favour_point, c.comment_id, u.profile_picture
