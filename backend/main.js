...
app.post('/api/register', async (req, resp) => {
    ...
    if (!results.rows.length <= 0) {
        ...
        resp.type('application/json');
        resp.json({
            message: `Username [${credentials.username}] already exists.`,
        });
        return;
    } else {
        try {
            ...
            console.info(
                `ERROR: Insert to user failed with following error: ${e}`,
            );
            resp.status(400);
            resp.type('application/json');
            resp.json({
        ...
        resp.type('application/json');
        resp.json({
            message: `Successfully created an account for ${credentials.username}!`,
        });
        return;
    }
});
...
app.post('/api/create_community', async (req, resp) => {
    ...
    try {
        ...
        if (e.code === '23505') {
            ...
            resp.type('application/json');
            resp.json({
                message: `Community name [${req.body.communityName}] already exists.`,
            });
            return;
            ...
            resp.type('application/json');
            resp.json({
                message: `Community name [${req.body.communityName}] failed check constraints.`,
            });
            return;
        }
        resp.status(400);
        resp.type('application/json');
        resp.json({
            message: `An error has occurred while creating community.`,
        });
        return;
    }
    resp.status(200);
...
app.get('/api/homepage_posts', async (req, resp) => {
    const results = await getHomePagePosts();
    if (results.rows.length == 0) {
        resp.status(204);
        resp.type('application/json');
        resp.json({ message: 'No posts found!' });
